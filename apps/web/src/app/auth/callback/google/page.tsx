...
export default function GoogleOAuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      try {
        ...
        console.log('Calling Google OAuth callback:', { code, state })
        ...
        if (error.details?.type === 'resurrection_available') {
          setResurrectionToken(error.details.resurrectionToken || null)
          setResurrectionEmail(error.details.oauthEmail || null)
          setResurrectionUserInfo(error.details.oauthUserInfo || null)
  ...
  const handleResurrectSubmit = async (action: 'accept' | 'decline') => {
    ...
    try {
      ...
      console.error('OAuth resurrect error:', error)
      setStatus('error')
      setMessage('Failed to complete account setup. Please try again.')
    } finally {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">
              {status === 'loading' && 'Completing Google Login...'}
              {status === 'success' && (isNewUser ? 'Welcome to Grateful!' : 'Welcome Back!')}
              {status === 'error' && 'Google Login Failed'}
            </h1>
          </div>
